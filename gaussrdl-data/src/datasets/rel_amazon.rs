...
const DATASET_URL: &str = "https://relational-datasets.s3.amazonaws.com/amazon";
...
impl RelAmazonDataset {
    ...
    fn load_users(&mut self) -> Result<()> {
        let file_path = self.download_file("users.parquet")?;
        let file = std::fs::File::open(file_path)?;
        let df = ParquetReader::new(file).finish()?;
        self.tables.insert("users".to_string(), df);
        Ok(())
    }
    ...
    fn load_interactions(&mut self) -> Result<()> {
        let file_path = self.download_file("interactions.parquet")?;
        let file = std::fs::File::open(file_path)?;
        let df = ParquetReader::new(file).finish()?;
        self.tables.insert("interactions".to_string(), df);
        Ok(())
    }
}

impl Dataset for RelAmazonDataset {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn validate(&self) -> Result<()> {
        ...
        if self.tables.is_empty() {
            return Err(gaussrdl_core::Error::validation("Dataset is empty"));
        }
        
        // Validate required tables
        let required_tables = ["users", "items", "interactions"];
...
mod tests {
    ...
    fn test_dataset_save_load() -> Result<()> {
        ...
        dataset.tables.insert("users".to_string(), df.clone());
        dataset.tables.insert("items".to_string(), df.clone());
        dataset.tables.insert("interactions".to_string(), df);
        
        // Save dataset
        dataset.save(temp_dir.path())?;
        
        // Load dataset
        let mut loaded = RelAmazonDataset::new();
