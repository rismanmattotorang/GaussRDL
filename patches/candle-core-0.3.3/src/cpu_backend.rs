...
impl BackendDevice for CpuDevice {
    ...
    fn rand_uniform(&self, shape: &Shape, dtype: DType, min: f64, max: f64) -> Result<CpuStorage> {
        ...
        let storage = match dtype {
            DType::U8 => {
                let uniform = rand::distributions::Uniform::new(min as u8, max as u8);
                let data = (0..len).map(|_| rng.sample(uniform)).collect();
                CpuStorage::U8(data)
            }
            DType::BF16 => {
                let uniform = rand::distributions::Uniform::new(min as f32, max as f32);
                let data = (0..len)
                    .map(|_| bf16::from_f32(rng.sample(uniform)))
                    .collect();
            ...
            DType::F16 => {
                let uniform = rand::distributions::Uniform::new(min as f32, max as f32);
                let data = (0..len)
                    .map(|_| f16::from_f32(rng.sample(uniform)))
                    .collect();
                CpuStorage::F16(data)
            }
            DType::F32 => {
            ...
            DType::U32 => {
                let uniform = rand::distributions::Uniform::new(min as u32, max as u32);
                let data = (0..len).map(|_| rng.sample(uniform)).collect();
                CpuStorage::U32(data)
            }
            DType::I64 => {
                let uniform = rand::distributions::Uniform::new(min as i64, max as i64);
                let data = (0..len).map(|_| rng.sample(uniform)).collect();
                CpuStorage::I64(data)
            }
        };
    ...
    fn rand_normal(&self, shape: &Shape, dtype: DType, mean: f64, std: f64) -> Result<CpuStorage> {
        ...
        let storage = match dtype {
            ...
            DType::BF16 => {
                let normal = rand_distr::Normal::new(mean as f32, std as f32).unwrap();
                let data = (0..len)
                    .map(|_| bf16::from_f32(rng.sample(normal)))
                    .collect();
                CpuStorage::BF16(data)
            }
            DType::F16 => {
                let normal = rand_distr::Normal::new(mean as f32, std as f32).unwrap();
                let data = (0..len)
                    .map(|_| f16::from_f32(rng.sample(normal)))
                    .collect();
                CpuStorage::F16(data)
            }
            DType::F32 => {
                let normal = rand_distr::Normal::new(mean as f32, std as f32).unwrap();
                let data = (0..len).map(|_| rng.sample(normal)).collect();
                CpuStorage::F32(data)
            }
            DType::F64 => {
