...
pub struct ModelConfig {
    ...
    pub num_walks: usize,
    pub walk_p: f32,  // Return parameter
    pub walk_q: f32,  // In-out parameter
...
impl ModelConfig {
        ...
        match self.distributed_strategy {
            DistributedStrategy::SingleDevice => {
                if self.world_size != 1 {
                    return Err(crate::ModelError::Config("world_size must be 1 for SingleDevice strategy".to_string()));
                }
            }
            DistributedStrategy::DataParallel | DistributedStrategy::ModelParallel | DistributedStrategy::PipelineParallel => {
                if self.world_size < 2 {
                    return Err(crate::ModelError::Config("world_size must be at least 2 for distributed strategies".to_string()));
                }
                if self.local_rank >= self.world_size {
                    return Err(crate::ModelError::Config("local_rank must be less than world_size".to_string()));
                }
            }
        }
        Ok(())
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.hidden_dim == 0 {
            return Err(crate::ModelError::Config("hidden_dim must be greater than 0".to_string()));
        }
        
        if self.hidden_dim % self.num_attention_heads != 0 {
            return Err(crate::ModelError::Config("hidden_dim must be divisible by num_attention_heads".to_string()));
        }
        
        // Validate learning parameters
        if self.learning_rate <= 0.0 || self.learning_rate >= 1.0 {
            return Err(crate::ModelError::Config("learning_rate must be between 0 and 1".to_string()));
        }
        
        if self.weight_decay < 0.0 {
            return Err(crate::ModelError::Config("weight_decay must be non-negative".to_string()));
        }
        
        if self.max_grad_norm <= 0.0 {
            return Err(crate::ModelError::Config("max_grad_norm must be positive".to_string()));
        }
        
        // Validate memory settings
        if self.max_memory_mb < 1024 {
            return Err(crate::ModelError::Config("max_memory_mb must be at least 1024".to_string()));
        }
    ...
    pub fn to_versioned_string(&self) -> Result<String> {
        #[derive(Serialize)]
    ...
    pub fn from_versioned_string(json: &str) -> Result<Self> {
        #[derive(Deserialize)]
        struct VersionedConfig {
            version: String,
            config: ModelConfig,
